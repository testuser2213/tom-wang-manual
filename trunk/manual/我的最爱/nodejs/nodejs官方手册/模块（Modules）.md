...
## 文件模块 ##
1、如果没有找到模块名对应的确切的文件名，node将试图在模块名后加上.js、.json、.node后缀（按优先级高低排序）来尝试加载  
...
1、当没有找到全局模块，同时模块名不是以“./”或“../”开头的时候执行  
...
- /path/to/node\_modules/b.js  
- /path/node\_modules/b.js  
- /node\_modules/b.js  
...
（2）、如果目录中没有`package.json`文件，require将在目录中尝试载入`index.js`或`index.node`，以（1）中的例子，如果没有package.json文件，nodejs会尝试载入：  
